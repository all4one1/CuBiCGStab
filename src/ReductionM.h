...
namespace CuCG
{
	...
	__device__ void extra_action__(double res, CuCG::ExtraAction action)
	{
		switch (action)
		{
		case CuCG::ExtraAction::compute_rs_new_and_beta:
			rs_new = res;
			beta = (rs_new / rs_old) * (alpha / omega);
			rs_old = rs_new;
			break;
		case CuCG::ExtraAction::compute_alpha:
			alpha = rs_new / res;
			break;
		case CuCG::ExtraAction::compute_omega:
			omega = buffer / res;
			break;
		case CuCG::ExtraAction::compute_buffer:
			buffer = res;
			break;
		case CuCG::ExtraAction::compute_rs_old:
			rs_old = res;
			break;
		default:
			break;
	...
	__global__ void reduce5(T* g_idata, T* second, T* g_odata, unsigned int n, bool first, bool last, CuCG::ExtraAction action) {
	...
	struct CudaReductionM
	{
		...
		double reduce(double* v1, double* v2, bool withCopy = true, CuCG::ExtraAction action = CuCG::ExtraAction::NONE);
		CuGraph make_graph(double* v1, double* v2, bool withCopy, CuCG::ExtraAction action);
	};
	...
	double CudaReductionM::reduce(double* v1, double* v2, bool withCopy, CuCG::ExtraAction action)
	{
		arr[0] = v1;	second = v2;
	...
	CuGraph CudaReductionM::make_graph(double* v1, double* v2, bool withCopy, CuCG::ExtraAction action)
	{
		arr[0] = v1;	second = v2;
		for (unsigned int i = 1; i <= steps; i++)
			arr[i] = res_array;
